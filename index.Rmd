...
Now that we can make vectors we can make subsets of bigger datasets, matrices and dataframes. Just to start off, we'll use a simple dataset which comes with R: `trees`.
...
```
We can access some important information using `$`:
```{r}
...
#soiltempdata <- transform(soiltempdata, Time=paste0(Year, "-", Month, "-", Day, " ", sprintf("%02i", Hour), ":00:00"))
...
# gvistempdata <- transform(soiltempdata, Time=paste0(sprintf("%02i", Hour), ":00 ", month.name[Month], " ", sprintf("%02i", Day, ", ", "Year")))
