...
```{r}
str(soiltempdata$X100.cm.Depth.Soil.Temperature....)
```
...
A slight problem (but one which will not force us to have to read the data in again) is that the Date column is a factor (has no real order) rather than a time object (which does have an order). We can remedy this quickly by assigning that column a copy of itself formatted correctly:

```{r}
str(soiltempdata$Date.Time..AST.)
soiltempdata$Date.Time..AST. <- strptime(soiltempdata$Date.Time..AST., "%m/%d/%Y %H:%M")
str(soiltempdata$Date.Time..AST.)
```
...
We can now do some basic plotting. Let's look at the 0cm depth over time. This is easy using the `plot` function. We'll use the `type="l"` argument to make it a line plot:

```{r, fig.width=14, fig.height=5}
plot(soiltempdata$Date.Time..AST., soiltempdata$X0.cm.Depth.Soil.Temperature...., type="l")
```
