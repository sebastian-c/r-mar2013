...
It's 3, because $\frac{6}{2} = 3$. Press 'p' for further explanation
...
Now that we can make vectors we can make subsets of bigger datasets, matrices and dataframes. Just to start off, we'll use a simple dataset which comes with R: `trees`. It is a data frame, a collection of vectors as columns. The vectors are allowed to be different types (character, integer, numeric).
...
A simple exploratory analysis could start, for example, with and histogram of the data to observe the distribution of the data. Try using the function `hist` with the temperatures at depth 0:
...
Other simple and useful plot is the boxplot. Using again the soil temperature at depth 0 and the function `boxplot`:
...
We can make your first plot a lot fancier if we want:
...
This presentation was entirely made in R using some useful packages such as [slidify](http://slidify.org). You can look at the source for it on [GitHub](https://github.com/sebastian-c/r-mar2013).
